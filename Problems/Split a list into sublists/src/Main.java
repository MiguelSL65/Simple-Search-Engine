...
class ListUtils {
    ...
    public static <T> List<List<T>> splitListIntoSubLists(List<T> list, int subListSize) {
        ...
        int size = list.size();
        int numOfListsFromOriginal = size / subListSize;

        if (size % subListSize != 0) {
            numOfListsFromOriginal++;
        }

        for (int i = 0; i < numOfListsFromOriginal; i++) {

            int fromIndex = i * subListSize;
            int toIndex = Math.min(i * subListSize + subListSize, size);
